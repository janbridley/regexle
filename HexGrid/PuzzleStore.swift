...
final class PuzzleStore: ObservableObject {
    ...
    func markSolved() {
        guard let lettersStr = progress.active[n],
              Self.solvesActive(in: progress, n: n, lettersString: lettersStr) else { return }
    ...
    private static func activeCounter(of p: ProgressV1, n: Int) -> Int { solvedCount(in: p, n: n) + 1 }
    ...
    private static func solvesActive(in p: ProgressV1, n: Int, lettersString: String) -> Bool {
        ...
        for i in 0..<generated.clues.count {
            ...
            guard line.count == cells.count,
                  RegexleGenerator.fullMatches(generated.clues[i], line) else { return false }
        }
        return true
    }
}
