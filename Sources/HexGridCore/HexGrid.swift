...
/// A hexagonal cluster of flat-top hexagons: a "hexagon of hexagons" with `n`
/// hexagons along each side (3n² − 3n + 1 cells in total).
///
/// Cells are addressed with axial coordinates (q, r); a cell belongs to the
/// cluster iff its cube radius max(|q|, |r|, |q+r|) ≤ n − 1. Every coordinate
/// is a linear function of `radius` (the center-to-vertex distance), so the
/// whole cluster scales uniformly — pass a radius derived from your target
/// size at draw time and it stays sharp at any resolution.
public struct HexGrid {
    ...
    ///   width  = s · (3n − 1)
    ///   height = s · √3 · (2n − 1)
    public var boundsWidth: Double  { radius * Double(3 * n - 1) }
    public var boundsHeight: Double { Self.sqrt3 * radius * Double(2 * n - 1) }
    ...
    public static func radiusFitting(
        ...
        let byWidth  = availableW / Double(3 * n - 1)
        let byHeight = availableH / (sqrt3 * Double(2 * n - 1))
        return Swift.min(byWidth, byHeight)
    }

    /// Axial coordinates of every cell in the cluster, in cube radius n−1.
    public func cells() -> [(q: Int, r: Int)] {
        ...
        for q in -k...k {
            // Cube constraint max(|q|, |r|, |q+r|) ≤ k  ⇔  r ∈ [max(-k, -q-k), min(k, -q+k)]
            let rMin = Swift.max(-k, -q - k)
            let rMax = Swift.min(k,  -q + k)
            for r in rMin...rMax { out.append((q, r)) }
        }
        return out
    }

    /// Pixel center of cell (q, r) for flat-top axial layout. The axial origin
    ...
    public func center(q: Int, r: Int, originX: Double = 0, originY: Double = 0) -> HexPoint {
        let cx = originX + radius * 1.5 * Double(q)
        let cy = originY + Self.sqrt3 * radius * (Double(r) + Double(q) / 2)
        return HexPoint(cx, cy)
    }

    /// The six vertices of a flat-top hexagon (angles 0°,60°,…,300°) centered
    /// at `c`, each at distance `radius` from the center.
    public func vertices(centeredAt c: HexPoint) -> [HexPoint] {
        (0..<6).map { k in
            let angle = Double(k) * Double.pi / 3
