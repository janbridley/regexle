...
public enum RegexleGenerator {
  ...
  public static func generate(
    ...
    for i in 0..<topo.clueEdges.count {
      ...
      clues.append(
        makeClue(
          for: fullStr, difficulty: difficulty,
  ...
  private static func makeClue(
    for fullStr: String, difficulty: Double,
    maxAttempts: Int, using rng: inout SFC64
  ) -> String {
  ...
  private static func randomRegex(
    for fullStr: String, difficulty: Double,
    using rng: inout SFC64
  ) -> String {
    let chars = Array(fullStr)
    let len = chars.count
    ...
    var rule = format(
      template, distractor: distractor, center: center,
  ...
  private static func noRepetitionRandomString(
    avoiding: [String], maxLen: Int,
    using rng: inout SFC64
  ) -> String {
  ...
  private static func format(
    _ template: String, distractor: String, center: String,
  ...
  /// Process-wide cache of compiled transform patterns.
  /// Generation is single-threaded, so no locking is needed.
  ...
  private static func subFirst(_ pattern: String, _ input: String, _ template: String) -> String {
    guard let regex = Self.regex(pattern) else { return input }
