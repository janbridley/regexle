...
final class HexGridCoreTests: XCTestCase {
    ...
    func testCellsLieWithinCubeRadiusAndReachEveryCorner() {
        ...
        for (q, r) in cells {
            XCTAssertLessThanOrEqual(max(abs(q), abs(r), abs(q + r)), k, "cell (\(q),\(r)) outside cluster")
        }
    ...
    func testBoundsAndRadiusFitting() {
        ...
        XCTAssertEqual(HexGrid.radiusFitting(n: 4, width: 100, height: 10_000, margin: 0),
                       100 / (HexGrid.sqrt3 * Double(2 * 4 - 1)), accuracy: 1e-9)
        XCTAssertEqual(HexGrid.radiusFitting(n: 4, width: 10_000, height: 100, margin: 0),
